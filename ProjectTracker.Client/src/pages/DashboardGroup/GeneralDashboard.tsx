...
import { RiskStatusArray, RiskStatusColors } from "@/types";
import type { AddMemberToProjectDto, CreateRiskDto, RiskStatus } from "@/types";
...
const GeneralDashboard = () => {
  ...
  const { selectedProject, addMember, addRisk } = useProjectsData({
    id: projectMetaData?.id,
  });

  const pmMember = selectedProject?.projectMembers.find(
    (member) => member.role == "ProjectManager",
  );
  const teamMembers = selectedProject?.projectMembers.filter(
    ...
    <div className="max-w-255 w-full mt-28">
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <div className="flex flex-col col-span-2 min-w-2.5 p-5 pr-7 border bg-card border-border rounded-md">
          <span className="text-sm text-muted-foreground">
            {`RISK REGISTER (${selectedProject?.risks.length ?? 0})`}
          </span>
          <Label className="pt-5 text-muted-foreground" htmlFor="risk-desc">
            Risk Description
          </Label>
          <div className="grid grid-cols-8 gap-4 mt-2.5">
            ...
            />
            <Select
              onValueChange={(status) => setNewRiskStatus(status as RiskStatus)}
            >
                  ...
                  {selectedProject.risks.map((risk, idx) => {
                    return (
                      <TableRow key={idx}>
                        ...
                        <TableCell>
                          <Badge
                            className={`w-22.5 h-5 ${RiskStatusColors[risk.status]["bgOpaque"]} 
                                                                        ${RiskStatusColors[risk.status]["text"]}`}
                          >
                            {risk.status}
                          </Badge>
                        </TableCell>
